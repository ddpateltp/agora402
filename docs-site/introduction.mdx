...
(listings, delistings)                (one receipt per settlement)
       ^   |                                     ^
       |   | read via mirror node                | publish receipt
       |   v                                     |
 Buyer agent  ──discover──►  Seller service
              ◄──quote──►    POST /a2a/quote
              ◄──402/pay──►  POST /v1/infer  |  GET /v1/rates/hbar
       │
       │ partially signed TransferTransaction
       ▼
 Blocky402 facilitator  ──►  Hedera testnet (consensus + HTS)
```
