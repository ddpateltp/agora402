...
/**
 * Provider abstraction shared by the paid inference endpoint and the audit
 * pipeline. Groq (OpenAI-compatible) for development, Anthropic for the
 * final demo, mock for tests and offline runs.
 */
export function createLlmProvider(opts: LlmOptions, fetchImpl: typeof fetch = fetch): LlmProvider {
  switch (opts.provider) {
    case 'groq':
      if (!opts.groqApiKey) throw new Error('provider groq requires groqApiKey');
      return openAiCompatible('groq', 'https://api.groq.com/openai/v1/chat/completions', opts.groqApiKey, opts.groqModel ?? 'openai/gpt-oss-20b', fetchImpl);
    case 'anthropic':
      if (!opts.anthropicApiKey) throw new Error('provider anthropic requires anthropicApiKey');
      return anthropic(opts.anthropicApiKey, opts.anthropicModel ?? 'claude-sonnet-4-5', fetchImpl);
    default:
      return mockProvider();
  }
}
